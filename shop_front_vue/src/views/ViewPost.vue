<template>
  <div class="center">
    ...
    <div class="right">
      ...
      <hr>
      <div class="edit">
      <vs-button size="xl" warn @click="buy(post.id,post.price)">立即购买</vs-button>
      </div>
      <div class="edit">
      <vs-button size="xl" danger @click="joinCollect">加入收藏</vs-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getPost() {
      ...
      this.postId = this.$route.path.split('product/')[1]
    ...
    async joinCollect(){
      ...
      form_data.append("pid",this.postId)
        const { data: res } = await this.$http.post(`collect/add`,form_data)
